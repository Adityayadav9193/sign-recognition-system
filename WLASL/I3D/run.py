...
def load_model(weights, num_classes):
  ...
  # nslt_2000_000700.pt nslt_1000_010800 nslt_300_005100.pt(best_results)  nslt_300_005500.pt(results_reported) nslt_2000_011400
    ...
    nlp_path = os.path.join(os.path.dirname(__file__), "NLP", "nlp_data_processed")
    nlp_gram_path = os.path.join(os.path.dirname(__file__), "NLP", "nlp_gram_counts")

    with open(nlp_path, "rb") as fp:   # Unpickling
           train_data_processed = pickle.load(fp)
    
    global n_gram_counts_list
    with open(nlp_gram_path, "rb") as fp:   # Unpickling
...
def create_WLASL_dictionary():
    ...
    with open('data/wlasl_class_list.txt') as file:
        for line in file:
